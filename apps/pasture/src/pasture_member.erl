...
-define(STATE,pasture_member_state).
-record(?STATE,{ member_id,
                 member }).
...
to_prop(Rec) ->
    Fields = record_info(fields, pasture_member),
...
rest_init(Req, _Opts) ->
    {MemberId,Req1} = case cowboy_req:binding(member_id, Req) of
        {undefined, R} ->
            {undefined, R};
        {MID,R} ->
            {binary_to_integer(MID),R}
    end,
    {ok, Req1, #?STATE{ member_id = MemberId } }.

resource_exists(Req,#?STATE{ member_id = undefined } = State) ->
...
resource_exists(Req,#?STATE{ member_id = Mid } = State) ->
    case mnesia:dirty_read(?MODULE,Mid) of
        []    -> {false,Req,State};
        [Rec] -> {true,Req,State#?STATE{ member = Rec }}
    end.
...
do_handle_json_path(Req,#?STATE{member_id = undefined} = State,
                    <<"GET">>, Path)
                    when Path =:= <<"/pasture_member">>;
                         Path =:= <<"/pasture_member/">> ->
    First = mnesia:dirty_first(?MODULE),
    {json_range(first,First),Req,State};
%% resource_exists will handle a invalid member_id ( First id )
do_handle_json_path(Req,#?STATE{member = _M, member_id = Mid} = State,
                    <<"GET">>, _Path) ->
...
json_range(_,First) ->
    %_ListFirst = integer_to_list(First),
    {Next,JsonRecs,_} = loop(next,First),
    ListNext = integer_to_list(Next),
...
json_struct(ListPrev,ListNext,JsonRecs) ->
    jsx:encode([{prev,list_to_binary("/pasture_member/"++ListPrev)},
                {next,list_to_binary("/pasture_member/"++ListNext)},
                {pasture_member,lists:reverse(JsonRecs)}
               ]).
